...
export class EmailEndpoint {
    ...
    private static async handleGet(action: string, id?: string, query?: any): Promise<Response> {
        switch (action) {
            case 'config':
    ...
    private static async handlePost(action: string, body: any): Promise<Response> {
        switch (action) {
            case 'logs':
                if (!body) {
    ...
    private static async handlePut(action: string, id: string | undefined, body: any): Promise<Response> {
        switch (action) {
            case 'config':
                if (!body) {
    ...
    private static async handleDelete(action: string, id?: string): Promise<Response> {
        switch (action) {
            case 'logs':
                if (id === 'all') {
    ...
    private static errorResponse(message: string, status: number = 400): Response {
        return {
            status,
            headers: {
                'Content-Type': 'application/json',
